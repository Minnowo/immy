...
#define L_D(...) immy_log(__LOG_LEVEL_DEBUG, stdout, __VA_ARGS__)
#define L_I(...) immy_log(__LOG_LEVEL_INFO, stdout, __VA_ARGS__)
#define L_W(...) immy_log(__LOG_LEVEL_WARN, stdout, __VA_ARGS__)
#define L_E(...) immy_log(__LOG_LEVEL_ERROR, stderr, __VA_ARGS__)
#define L_C(...) immy_log(__LOG_LEVEL_CRITICAL, stderr, __VA_ARGS__)
...
typedef struct immy_control {
        ...
        immy_image_t* selected_image; // the selected image;
                                      // points to the image_files;

        immy_message_t message;      // message to show to the user
        dimage_arr_t   image_files;  // images files loaded or not
        immy_config_t  config;       // runtime settings
...
// string functions
// 
char* immyStrdup(const char* str);  // just regular strdup
char* immyEstrdup(const char* str); // strdup but dies instead of returning null
...
char* immyGetThumbPath(const immy_config_t* conf, const char* imagePath);
 ...
 */
void immy_raylib_log(int msgType, const char *text, va_list args);
 ...
 */
bool immy_copy_and_resize_image_nn(const  Image* image, Image* newimage, int newWidth, int newHeight);

/**
 * Create a thumbnail image from the given image
 */
bool immyCreateThumbnail(const Image* image, Image* newimage, int newWidth, int newHeight);
...
bool  immyCreateDirectory(const char* path);
 ...
 */
void immySetCacheDir(immy_config_t* conf, const char* path);
