...
void generate_resource_bundle(void)
{
    ...
    if(!out) { 
        ...
        fprintf(stderr, "Could not read %s: %s", BUNDLE_H_PATH, strerror(errno));
    ...
    for (size_t i = 0; i < RESOURCES_COUNT; ++i) {
        ...
        if(!f) {
            ...
            fprintf(stderr, "Could not read %s: %s", resources[i].file_path, strerror(errno));
    ...
    for (size_t i = 0; i < RESOURCES_COUNT; ++i) {
        ...
        while (read == 1)
        {
            ...
            if (read)
                genfp(out, "0x%02X,", (unsigned char)c1);
